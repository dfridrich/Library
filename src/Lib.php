...
class Lib
{
    ...
    public static function identifyImage($filename)
    {
        ...
        if ($data['dpi_x'] == 10752) $data['dpi_x'] = 72;
        if ($data['dpi_y'] == 10752) $data['dpi_y'] = 72;
    ...
    {
        if ($origDpi == 0 or $printW == 0 or $origW == 0) return 0;
    ...
    public static function normalizeKeywords($string)
    {
        ...
        foreach ($string as $word) {
            if (is_numeric($word)) {
                ...
                $len = strlen($word);
                if ($len >= 7) $word = substr($word, 0, -3);
                elseif ($len > 3) $word = substr($word, 0, -1);
                $out[] = $word;
            }
        }
